 ...
 * + Eliminar definitivo SOLO si está cancelada (borra lineareserva y luego reserva)
 *
...
/**
 * Acciones:
 * - confirmar: envía email por SMTP -> si OK, estado = confirmada
 * - cancelar:  estado = cancelada
 * - eliminar:  SOLO si estado = cancelada:
 *      borra lineareserva (por reserva_id) y luego reserva
 *
 * Nota:
 * - Se ejecuta solo en tabla=reserva (evita colisiones).
 * - return_url: vuelve a la misma vista con filtros/orden.
 */
if ($tabla === "reserva" && isset($_POST["accion_reserva"])) {
  ...
  if ($reserva_id <= 0) {
    ...
    if ($accion_reserva === "confirmar") {
      ...
      // No confirmamos una cancelada
      if ($estado_actual === "cancelada") {
        js_redirect($return_url . "&ok=estado_no_permitido");
      }

      // Sin pass, no intentamos
      if (trim((string)$SMTP_PASS) === "") {
        notice_html(
          "<strong>No se pudo enviar el correo.</strong><br>Falta configurar <code>\$SMTP_PASS</code> (en Gmail suele ser una <em>App Password</em>).<br><strong>No se confirmó</strong> la reserva.",
          "err"
        );
      } else {

        // Datos para el email (tomamos 1 línea)
        $sqlInfo = "
          SELECT
            r.id AS reserva_id,
            r.fecha AS fecha,
        ...
        if (!$info) {
          ...
          if ($to === "") {
            notice_html("<strong>No se pudo enviar el correo.</strong><br>El cliente no tiene email registrado.<br><strong>No se confirmó</strong> la reserva.", "err");
    ...
    /* ============ ELIMINAR DEFINITIVO ============ */
      ...
      try {
        ...
        // 2) Borrar reserva (solo cancelada)
...
if ($tabla === "reserva" && isset($_POST["crear_reserva"])) {
  ...
  if (count($errores) > 0) {
    ...
    try {
      ...
      // 2) Insert RESERVA (estado inicial pendiente si existe)
      if ($HAS_ESTADO) {
...
/* ============================
   10) RESERVAS — EDITAR / ACTUALIZAR (reserva + línea)
   ============================ */
if ($tabla === "reserva" && isset($_POST["guardar_edicion"])) {
  ...
  if (count($errores) > 0) {
    ...
    $cn->begin_transaction();
    try {
      // No tocamos estado aquí (solo fecha/cliente)
...
if ($tabla === "reserva" && $accion === "ver") {
  ...
  if (!$row) {
    notice_html("No encontrado.", "warn");
    return;
  }
...
/* ============================
   17) CLIENTES — listado + filtro + orden + botón nuevo
   ============================ */
if ($tabla === "cliente") {
...
/* ============================
   18) RESERVAS — listado + filtros + orden + acciones
   ============================ */
if ($tabla === "reserva") {
  ...
  // URL de retorno (mantiene filtros/orden)
  ...
  if ($res && $res->num_rows > 0) {
    while ($r = $res->fetch_assoc()) {
      ...
      // Acciones por reserva (requieren estado)
